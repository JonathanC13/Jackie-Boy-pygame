...
For example, I had one collision problem where while going up a ramp the player encounters a tile above them, but they were able to phase through it. The reason was the calculation for the player's y position to 'stand' on the ramp caused the player to surpass the above tile's collision threshold y position. The threshold contains two components, collision True/False and side of approach.
<br>
