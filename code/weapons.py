...
class Stick(Weapon, Orbit):
    def __init__(self, pos, groups, frames, owner, damage, damage_type, level, attack_cooldown, attack_speed):
        ...
        super().__init__(damage = damage, damage_type = damage_type, level = level, attack_cooldown = attack_cooldown, attack_speed = attack_speed,
                         pos = pos, frames = frames, radius = self.radius, speed = 1, start_angle = 0, end_angle = -1, groups = groups, type = STICK, z = Z_LAYERS['main'], direction_changes= -1, rotate = True, image_orientation = IMAGE_RIGHT
                         )
        
    def check_in_range(self, player_sprite):
        # weapon range
        weapon_range_rect = pygame.FRect(self.owner.hitbox_rect.center - pygame.Vector2(self.range, self.range), (self.range * 2, self.range * 2))
        weapon_range_rect_center = weapon_range_rect.center
        pygame.draw.rect(pygame.display.get_surface(), "green", weapon_range_rect)

        if (weapon_range_rect.colliderect(player_sprite.hitbox_rect)):
            left_offset = 0
            slice_offset = 0
            # circle
            # x pos into the weapon_range_rect and then - self.range to get relative from center
            rel_x = player_sprite.rect.x - weapon_range_rect.x - self.range
            if (player_sprite.rect.centerx < weapon_range_rect_center[0]):
                # from left
                left_offset = player_sprite.rect.width
                slice_offset = -1
            ...
            if (rel_x < self.range):
                ...
                weapon_range_slice_rect = pygame.FRect((weapon_range_rect.x + self.range + rel_x + slice_offset, weapon_range_rect_center[1] - rel_y), (1, rel_y * 2))
                pygame.draw.rect(pygame.display.get_surface(), "red", weapon_range_slice_rect)
                if (weapon_range_slice_rect.colliderect(player_sprite.rect)):
                    self.owner.player_proximity["weapon_in_range"] = True
                    print(self.owner.player_proximity["weapon_in_range"])
