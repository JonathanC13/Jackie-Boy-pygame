...
# todo, parent class for overlays
...
class MainMenu:
    def __init__(self, overlay, font, save_data, overlay_frames):
        ...
        self.container_size = vector(250, 150)
    ...
    def create_container_save_data(self, save):
        ...
        # file name
        filename_surf = self.font.render('Filename: ' + str(save['filename']), False, "white", bgcolor=None, wraplength=0)
        ...
        container.append({"surf": heart_surf, "layer": 1, "topleft": vector((WINDOW_WIDTH/2) - self.container_size.x/2 + x_spacing, self.current_total_spacing_y + filename_surf.get_height() + y_spacing)})
        ...
        container.append({"surf": heart_num_surf, "layer": 1, "topleft": vector((WINDOW_WIDTH/2) - self.container_size.x/2 + x_offset, self.current_total_spacing_y + filename_surf.get_height() + y_spacing)})
        x_offset += heart_num_surf.get_width()  + x_spacing

        # denta
        denta_surf = self.overlay_frames['denta'][0]
        container.append({"surf": denta_surf, "layer": 1, "topleft": vector((WINDOW_WIDTH/2) - self.container_size.x/2 + x_offset, self.current_total_spacing_y + filename_surf.get_height() + y_spacing)})
        ...
        container.append({"surf": denta_num_surf, "layer": 1, "topleft": vector((WINDOW_WIDTH/2) - self.container_size.x/2 + x_offset, self.current_total_spacing_y + filename_surf.get_height() + y_spacing)})
        x_offset += denta_num_surf.get_width()  + x_spacing

        # kibble
        kibble_surf = self.overlay_frames['kibble'][0]
        container.append({"surf": kibble_surf, "layer": 1, "topleft": vector((WINDOW_WIDTH/2) - self.container_size.x/2 + x_offset, self.current_total_spacing_y + filename_surf.get_height() + y_spacing)})
        ...
        container.append({"surf": kibble_num_surf, "layer": 1, "topleft": vector((WINDOW_WIDTH/2) - self.container_size.x/2 + x_offset, self.current_total_spacing_y + filename_surf.get_height() + y_spacing)})
